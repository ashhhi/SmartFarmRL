...
# to predict the next state (for unknown state)
class DynamicsModel(nn.Module):
    def __init__(self, n_s, n_a):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(n_s + n_a, 128),
            nn.ReLU(),
            nn.Linear(128, n_s)
        )
...
class Critic(nn.Module):
    def __init__(self, n_s, n_a):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(n_s + n_a, 88),  # 输入状态和动作的大小
            nn.Tanh(),
            nn.Linear(88, 1)  # 输出 Q 值
        )
...
class Agent:
    def __init__(self, n_input, n_output):
        ...
        self.dynamics_model = DynamicsModel(n_s=n_input, n_a=n_output)
        ...
        self.optimizer_model = torch.optim.Adam(self.dynamics_model.parameters(), lr=0.001)
